...
The traditional Entity-Relationship (E-R) Diagram visually models the core relational architecture of ThriftThreads. It strictly follows standard notation rules: Entities are represented in **rectangles**, Attributes in **ovals**, and the logical bridging Relationship in a **diamond** accompanied by explicit cardinality (**1-N**) bindings.
...
flowchart TD
    ...
    P_price([price])
    P_img([image])
    P_seller([seller_id FK])

    %% Connecting User Attributes
    USER --- U_id
    USER --- U_name
    USER --- U_email
    USER --- U_pass
...
flowchart TD
    ...
    %% Primary Actions spread out horizontally
    User --> Register
    User --> Login
    User --> Browse
    User --> Sell

    %% Secondary Actions flow hierarchically downwards (stretching vertically and horizontally)
    Browse ---> Cart
    Cart ---> Checkout
...
#### 5. Class Diagram
The Class Diagram maps the static, object-oriented structure of the ThriftThreads ecosystem. It details the core application entities (`User`, [Product](file:///d:/ThriftThreadsIn/ThriftThreads%20Simple/ThriftThreads_Auth_Implementation/public/product.html#40-80), and `ShoppingCart`), explicitly defining their data properties, inherent state behaviors (methods), and structural cardinality relationships.
...
classDiagram
    ...
    User "1" -- "1" ShoppingCart : manages
    ShoppingCart "1" o-- "*" Product : aggregates
```
